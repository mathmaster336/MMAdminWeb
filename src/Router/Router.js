...
const Router = () => {
  ...
  const CoureseContent = Loadable(lazy(() => import("../Pages/CoureseContent")))
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const token = getLocalStorage("token");

  const hasVerifiedRef = useRef(false); // 👈 Used to ensure single API call
  useEffect(() => {
    if (!hasVerifiedRef.current) {
      ...
      (async () => {
        ...
        try {
          const res = await MMapi.post("/auth/admintokenverify", {});
  ...
  const routes = useRoutes([
    {
      ...
      children: !isAuthenticated && [
        ...
        { path: "/login", element: <Login hasVerifiedRef={hasVerifiedRef} /> },
        {
          path: "/register",
          element: <Register />,
        },
